...
actions, and rewards.
  ...
  """
  def __init__(self, obs_shape, rng, max_steps=1000, phi_length=1):
    """Construct a DataSet.
    ...
    self.imgs = np.zeros((max_steps, obs_shape), dtype='float32')
...
next_states for batch_size randomly chosen state transitions.
    ...
    while count < batch_size:
      ...
      next_states[count] = self.imgs.take(transition_indices,
                        ...
                        mode='wrap')
      count += 1
    return states, actions, np.asarray(rewards, dtype=np.float32), next_states, terminal
